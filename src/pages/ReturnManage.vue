                ...
                <div class="modal-dialog modal-md" role="document">
                    <div class="modal-content">
                        <div class="modal-header d-flex">
                            <div class="col-1"></div>
                            <h5 class="modal-title col-10" id="staticModalLabel">添加退货信息</h5>
                        ...
                        <div class="modal-body">
                          <div class="form-group">
                            <label for="vat" class=" form-control-label" align="left">销量订单</label>
                            <select name="select" id="addSelect" class="form-control" >
                                <option v-if="allMaterial.state" v-for="allMaterial in allMaterials" :value="allMaterial.id">id:{{allMaterial.id}}&emsp;日期:{{allMaterial.date}}</option>
                            </select>
                          </div>
                          <div class="form-group"><label for="country" class=" form-control-label" align="left">退货原因</label><input type="text" id="productQuantityInput" placeholder="请输入退货原因" class="form-control" ></div>
                        </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      getProducts(){
        //发送请求
        api.showReturnInfo("获取所有的退货信息失败").then(res => {
          console.log(res)
       ...
       */
      addProductType(){
        var salesId= $("#addSelect option:selected").attr("value");
        var reason=$('#productQuantityInput').val();

        //封装数据
        var obj={salesId:salesId,reason:reason};
        console.log(obj)
        //发送请求
        api.addReturn(obj,"添加退货信息失败").then(res=>{
          if(res.data.status==0){
            this.getProducts();
            this.getProductLists(1)
            $('#addModal').modal('hide');
      ...
      getAllMaterials(){
        //发送请求
        api.showSalesInfo("获取所有订单信息失败").then(res=>{
          console.log(res)
          if(res.data.status==0){

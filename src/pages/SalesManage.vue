                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                      ...
                      <div class="col-4 d-flex justify-content-end p-0">
                        ...
                        <button v-on:click="modifyCommodity" type="button" class="btn  mb-1 iconBtn" disabled="true">
                            <i class="menu-icon fa fa-cogs"></i>
                        </button>
                        <div class="d-flex justify-content-between" id="modifyCol" style="display:none!important;">
                          <p class="col-3 modifyColItem" style="padding-top:3px">订单编号:</p>
                    ...
                    <table class="table ">
                        ...
                        <tbody v-for="showOrderList,index in showOrderLists">
                          <tr >
                              ...
                              <td>{{showOrderList.price}}</td>
                              <td>{{showOrderList.totalPrice}}</td>
                              <td>{{showOrderList.date}}</td>
                              <td >
                                <span v-if="showOrderList.state" class="badge badge-success">已完成</span>
                                <span v-if="!showOrderList.state" class="badge badge-warning">已退货</span>
                              </td>
                ...
                <div class="modal-dialog modal-md" role="document">
                    <div class="modal-content">
                        <div class="modal-header d-flex">
                            <div class="col-1"></div>
                            <h5 class="modal-title col-10" id="staticModalLabel">添加销量信息</h5>
                        ...
                        <div class="modal-body">
                          <div class="form-group">
                            <label for="vat" class=" form-control-label" align="left">库存成品</label>
                            <select name="select" id="addSelect" class="form-control" >
                                <option v-for="allMaterial in allMaterials" :value="allMaterial.productId" :data-num="allMaterial.num">id:{{allMaterial.productId}}&emsp;名称:{{allMaterial.name}}&emsp;数量:{{allMaterial.num}}&emsp;日期:{{allMaterial.madeTime}}</option>
                            </select>
                          </div>
                          <div class="form-group"><label for="country" class=" form-control-label" align="left">数量</label><input type="text" id="productQuantityInput" placeholder="(必填)" class="form-control" oninput = "value=value.replace(/[^\d]/g,'')"></div>
                        </div>
                        ...
                        <div class="modal-body">
                          ...
                          <div class="form-group">
                            ...
                            <select name="select" id="updateSelect" class="form-control" multiple="multiple">
                                <option v-for="allMaterial in allMaterials" :value="allMaterial.id">id:{{allMaterial.id}}&emsp;名称:{{allMaterial.productName}}</option>
                            </select>
                          </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      getProducts(){
        //发送请求
        api.showSalesInfo("获取所有的销量订单信息失败").then(res => {
          console.log(res)
       ...
       */
      addProductType(){
        var productId= $("#addSelect option:selected").attr("value");
        var productNum=$("#addSelect option:selected").attr("data-num");
        var quantity=$('#productQuantityInput').val();
        console.log(productNum)
        console.log(quantity)
        if(parseInt(quantity)>parseInt(productNum)){
          alert("数量已超出库存，请重新填写！")
          return;
        }

        //封装数据
        var obj={productId:productId,num:quantity};
        console.log(obj)
        //发送请求
        api.addSales(obj,"添加销量信息失败").then(res=>{
          if(res.data.status==0){
            this.getProducts();
            this.getProductLists(1)
            $('#addModal').modal('hide');
      ...
      getAllMaterials(){
        //发送请求
        api.showProductRepository("获取库存成品失败").then(res=>{
          console.log(res)
          if(res.data.status==0){
      ...
      updateProductType(){
        ...
        //封装数据
        var obj={id:this.selectProductId,name:name,materials:materials};
        console.log(obj)
        //发送请求

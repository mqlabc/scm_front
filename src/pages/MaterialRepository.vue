                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                          <div class="form-group">
                            <label for="vat" class=" form-control-label" align="left">请选择原料</label>
                            <select name="select" id="addSelect" class="form-control">
                                <option v-for="allProduct in allProducts" :data-id="allProduct.id" :data-name="allProduct.rawMaterialName">id:{{allProduct.id}}&emsp;名称:{{allProduct.rawMaterialName}}</option>
                            </select>
                          </div>
                ...
                <div class="modal-dialog modal-md" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="mediumModalLabel">原料库存修改</h5>
            ...
            <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="staticModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-sm" role="document">
                        <div class="modal-content">
                            <div class="modal-header d-flex">
                                <div class="col-1"></div>
                                <h5 class="modal-title col-10" id="staticModalLabel">删除原料库存</h5>
                                <button type="button" class="close col-1 p-0 m-0" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <p>
                                   确定要删除该原料吗？<br><small class=" mt-2">（库存编号：{{selectProductId}}）</small>
                               </p>
                           </div>
                           <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">取消</button>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      getProducts(){
        //发送请求
        api.showMaterialRepository("获取所有的原料库存失败").then(res => {
          console.log(res)
      ...
      addProduct(){
        ...
        //封装数据
        var obj={rawMaterialId:id,rawMaterialName:name,quantity:quantity};
        console.log(obj)
        //发送请求
        api.addMaterialRepository(obj,"添加原料库存失败").then(res=>{
          if(res.data.status==0){
            this.getProducts();
            this.getProductLists(1);
            $('#addModal').modal('hide');
       ...
       */
      deleteProductType(){
        var obj={rawMaterialId:this.selectProductId};
        //发送请求
        api.deleteMaterialRepository(obj,"删除原料库存失败").then(res=>{
          if(res.data.status==0){
            //更新页面数据
            this.getProducts();
            this.getProductLists(1);
      ...
      updateProductType(){
        ...
        console.log(obj)
        //发送请求
        api.updateMaterialRepository(obj,"更新原料库存数量失败").then(res=>{
          if(res.data.status==0){
            //更新页面数据
            this.getProducts();
            this.getProductLists(1);
